...
class MySQL {
  ...
  constructor (url, defs) {
    ...
    this.con = null
    this.pool = mysql.createPool(this.url + (/\?/.test(this.url) ? '&' : '?') + 'multipleStatements=true')
  }
  ...
  /**
   * Create tables
   * @param {array} collections list name of collections for importing
  ...
  createTable (collections) {
    ...
    const sql2 = this.defs.filter(def => collections.length === 0 || collections.includes(def.name)).map(def => {
  ...
  closeAllConnection() {
    this.pool.end(err => util.log(err))
  }
